...
@Component({
  ...
  standalone: true,
  imports: [TuiButton, TranslatePipe, TuiIcon],
  template: `
    <div
    ...
    >
      ...
      <div class="flex flex-col items-center justify-center px-6 pt-8 pb-4">
        <tui-icon
          [icon]="context.data.danger ? 'triangle-alert' : '@tui.check'"
          class="mb-4 text-5xl"
      ...
      <div class="px-6 py-6">
        <div class="flex w-full flex-col items-stretch gap-3 sm:flex-row sm:items-center sm:justify-center">
          <button
            ...
            (click)="this.context.completeWith(true)"
...
export class InformationModalUiComponent {
  readonly context = injectContext<TuiDialogContext<boolean, InformationData>>();
}
